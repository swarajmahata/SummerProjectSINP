...
v_E = 232e3
v_esc = 544e3
E_R_th_default = 0.38  # keV (updated)
E_R_th_seitz = 3.84    # keV (updated)
...
sigma_limits_default = [3.0 / (R * exposure) if R > 0 else np.nan for R in rates_default]
sigma_limits_seitz = [3.0 / (R * exposure) if R > 0 else np.nan for R in rates_seitz]

# --- Print Rexp and cross-section values ---
print("Mass (GeV)\tR_exp (kg^-1 day^-1)\tSigma (pb)")
for m_x, R, sigma in zip(m_x_vals, rates_seitz, sigma_limits_seitz):
    print(f"{m_x:.2f}\t{R:.4e}\t{sigma:.4e}")

# --- Plot ---
plt.figure()
plt.plot(m_x_vals, sigma_limits_default, linestyle='--', color='blue', label=r"$T = 35^\circ$C, $\eta_T = 100\%$")
plt.plot(m_x_vals, sigma_limits_seitz, linestyle='-', color='red', linewidth=2, label=r"$T = 55^\circ$C, $\eta_T = 100\%$")
plt.xlabel("Mass (GeV)")
plt.ylabel(r"$\sigma^{\mathrm{SI}}_{\chi,n,95}$ (pb)")
plt.yscale("log")
plt.xlim(0, 5)
plt.ylim(1e-9, 1e-1)
plt.grid(True)
plt.title(r"Exposure = 1000 kg.day")
plt.legend()
plt.tight_layout()
