...
for target, color in zip(targets, colors):
    dRdE = calcSpectrum(target, operator=1, spin=0)
...
ax.set_title(r"Normalized $\frac{dR}{dE_R}$ for Operator $\mathcal{O}_1$")
ax.set_ylim(0, 0.06)
ax.legend(fancybox=True, fontsize=12)
plt.tight_layout()
os.makedirs("../plots", exist_ok=True)
plt.savefig(f"../plots/Spectrum_Operator1_Spin0_mx={int(m_x)}GeV.pdf", bbox_inches="tight")
plt.show()
...
plt.title("Exclusion Cross Section vs DM Mass (Operator $\\mathcal{O}_1$)")
...
plt.savefig("../plots/Sigma_vs_Mass_AllTargets.png", dpi=300)
plt.show()
